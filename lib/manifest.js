...
Manifest.prototype.free = function(service) {
  ...
  if (service.id && serviceExists.call(this, service) && this._services[service.id] && this._services[service.id].owner === this.id) { // lookup by id, make sure we own the service
...
Manifest.prototype.get = function(role) {
  ...
  debug('get services: %j', services);
  if (typeof role === 'object') return _.where(services, role);
  if (typeof role === 'string') {
    var lookup = {};
    var spl = role.split('@');
    lookup.role = spl[0];
